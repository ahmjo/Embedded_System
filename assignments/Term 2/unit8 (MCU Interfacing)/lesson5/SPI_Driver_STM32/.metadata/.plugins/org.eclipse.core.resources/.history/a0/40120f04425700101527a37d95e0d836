...
void MCAL_UART_ReceiveData(USART_t* USARTx, uint16_t* pRxBuffer, enum Polling_mechism PollingEn )
{
	...
	if(USARTx == USART1)
	{
		if(Global_UART1_Config->Payload_Length == UART_Payload_Length_9B)
		{
			if(Global_UART1_Config->parity == UART_parity_NONE)
			{
	...
	{
		if(Global_UART2_Config->Payload_Length == UART_Payload_Length_9B)
		{
			if(Global_UART2_Config->parity == UART_parity_NONE)
			{
				*((uint16_t*) pRxBuffer) = USARTx->DR;
			}
			else
	...
	{
		if(Global_UART3_Config->Payload_Length == UART_Payload_Length_9B)
		{
			if(Global_UART3_Config->parity == UART_parity_NONE)
			{
				*((uint16_t*) pRxBuffer) = USARTx->DR;
			}
			else
